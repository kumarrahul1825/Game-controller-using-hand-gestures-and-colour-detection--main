...
# allow the camera or video file to warm upDSDSDSDSDS
time.sleep(2.0)
initial = True
flag = False
...
while True:
    ...
    if len(cnts_left) > 0:
        ...
        if radius > circle_radius:
            ...
            cv2.circle(frame, (int(x), int(y)), int(circle_radius),
                (0, 255, 255), 2)
            cv2.circle(frame, center_left, 2, (0, 0, 255), -1)
            ...
            if center_left[1] < (height/2 - windowSize//2):
                cv2.putText(frame,'Fight',(20,50),cv2.FONT_HERSHEY_SCRIPT_COMPLEX,1,(0,0,255))
                PressKey(A)
    ...
    if len(cnts_right) > 0:
        ...
        if radius2 > circle_radius:
            # draw the circle and centroid on the frame,
            cv2.circle(frame, (int(x2)+width//2, int(y2)), int(circle_radius),
                (0, 255, 255), 2)
            cv2.circle(frame, center_right, 5, (0, 0, 255), -1)
            if center_right[1] < (height//2 - windowSize//2):
                cv2.putText(frame,'Fight',(200,50),cv2.FONT_HERSHEY_SCRIPT_COMPLEX,1,(0,0,205))
                PressKey(W)
                keyPressed = True
